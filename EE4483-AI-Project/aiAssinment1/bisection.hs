...
bisection 0 = [0]
bisection x | x >= 1    = loop (Range ((x + 1) / 2) 1) [] ((x + 1) / 2)
...
guessIsGoodEnough oldGuess newGuess =
  let precision = 0.0000001 in abs (oldGuess - newGuess) < precision

main :: IO ()
main = putStrLn "Please input number" >> getLine >>= \input ->
  mapM_ print . bisection $ (read $ input :: Double)
